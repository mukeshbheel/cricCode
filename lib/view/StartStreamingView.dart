...
import 'package:cricket_commentary/model/StreamParamsModel.dart';
import 'package:cricket_commentary/utils/StreamConstants.dart';
...
class StartStreamingView extends StatefulWidget {
  const StartStreamingView({Key? key}) : super(key: key);

  @override
  _StartStreamingViewState createState() => new _StartStreamingViewState();
}

class _StartStreamingViewState extends State<StartStreamingView>
    with WidgetsBindingObserver {
  final ButtonStyle buttonStyle =
  ElevatedButton.styleFrom(textStyle: const TextStyle(fontSize: 20));
  StreamParamsModel config = StreamParamsModel();
  ...
  ApiVideoLiveStreamController createLiveStreamController() {
    return ApiVideoLiveStreamController(
        initialAudioConfig: config.audioConfig,
        initialVideoConfig: config.videoConfig,
        onConnectionSuccess: () {
          ...
          actions: <Widget>[
            PopupMenuButton<String>(
              ...
              itemBuilder: (BuildContext context) {
                return StreamConstants.choices.map((String choice) {
  ...
  void _onMenuSelected(String choice, BuildContext context) {
    if (choice == StreamConstants.Settings) {
